...
export const withPage = ({ page = 1, pageSize = 20, ...query }) => ({
  page,
  pageSize,
  ...query
});
...
const getToken = (refresh = false) => {
  console.log("scopes", config.ct.auth.scopes);
  const storageToken = JSON.parse(localStorage.getItem("session"))?.tokenInfo;
...
export const toUrl = (base, { query: { pageSize, page, ...query } }) => {
  const queryParams = Object.entries(query).filter(([, v]) => v !== undefined);
